...
public class ChestUnlockSubCommand {
                                ...
                                try {
                                    if (ClansStorageUtil.removeProtectedChest(clanOwnerUUIDString, location, player)) {
                                        ...
                                        foliaLib.getImpl().runAsync((task) -> {
                                            fireAsyncChestUnlockEvent(player, location);
                                            MessageUtils.sendDebugConsole("Fired AsyncChestUnlockEvent");
                                        });
                                    ...
                                    MessageUtils.sendConsole("severe", messagesConfig.getString("clans-update-error-1"));
                                    MessageUtils.sendConsole("severe", messagesConfig.getString("clans-update-error-2"));
                                    e.printStackTrace();
                                }
                                return true;
                            }
    ...
    private void fireAsyncChestUnlockEvent(Player player, Location removedLockLocation) {
        AsyncChestUnlockEvent asyncChestUnlockEvent = new AsyncChestUnlockEvent(true, player, removedLockLocation);
        Bukkit.getPluginManager().callEvent(asyncChestUnlockEvent);
    }
}
